...
def get_neg_pair(lni_content,lni_sentance,topk):
    ...
    for index in lni_sentance.index:
        ...
        else: neg_train_data = None
    return neg_train_data

pos_train_date_2.drop_duplicates(inplace=True)
print('after dropping duplicated rows, remaining pos_train_date shape is {}'.format(pos_train_date_2.shape))
step = 5000
...
for epoch in range(len(lni_content.index)//100):
    ...
    if neg_train_data.shape[0] > THRESHHOLD: 
        ...
        # print('{} negative samples are created'.format(neg_train_data.shape[0]))
        break
neg_train_data.to_pickle('/data/yuchen/projects/sentence_embedding/data/train/neg_train_data_2_v0_million.pkl')
print('neg_train_data is stored, cost {} s'.format(time() - start_time))
